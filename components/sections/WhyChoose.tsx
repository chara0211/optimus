  ...
  return (
    <section className="mx-auto max-w-6xl px-4 pt-20">
      <div className="grid gap-10 md:grid-cols-2 md:items-center">
        <div>
          <h2 className="text-5xl font-extrabold leading-tight text-fg">Why Choose Optimus?</h2>
          ...
          <ul className="mt-8 space-y-4 text-muted">
            {points.map((p) => (
              <li key={p} className="flex gap-3">
                <span className="mt-[2px] grid h-6 w-6 place-items-center rounded-full bg-blue-500/15 text-blue-500">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {cards.map((c) => (
            <div key={c.bottom} className="card soft-shadow rounded-2xl p-7">
              <c.Icon className="h-7 w-7 text-blue-500" />
              <div className="mt-5 text-4xl font-extrabold text-fg">{c.top}</div>
              <div className="mt-2 text-sm text-muted">{c.bottom}</div>
            </div>
          ))}
        </div>
      </div>
